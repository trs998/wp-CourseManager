...
class yvts_courseRunning {
  ...
  static function getCoursesRunning($year = -1) {
    ...
    $sql = $wpdb->prepare( "SELECT `courseRunning_ID`,`$table_name_courseRunning`.`edittime`,`$table_name_courseRunning`.`note`,`starttime`,(UNIX_TIMESTAMP(`starttime`)+7200) as `starttimeU`,`endtime`,(UNIX_TIMESTAMP(`endtime`)+7200) as `endtimeU`, (((UNIX_TIMESTAMP(`endtime`) - UNIX_TIMESTAMP(`starttime`))/86400)+1) AS `days`, `note`,`$table_name_levels`.`name` AS `levelname`,`$table_name_levels`.`levelid` AS `levelid`,`$table_name_levels`.`levelprice` AS `levelprice`, `$table_name_courses`.`name` AS `coursename`, `$table_name_courses`.`description` AS `coursedesc` FROM `$table_name_courseRunning`
